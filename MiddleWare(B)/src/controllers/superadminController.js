...
const getAllUsers = async (req, res) => {
  try {
    ...
    console.log(`✅ Processed ${userStats.length} users with stats`);
...
const createTherapist = async (req, res) => {
  try {
    ...
    const therapist = new User({
      ...
      licenseNumber: specialization || 'ADMIN_CREATED',
      isActive: true
    });
...
const getSystemStats = async (req, res) => {
  try {
    ...
    const totalTherapists = await User.countDocuments({ userType: 'therapist' });
    const totalPatients = await User.countDocuments({ userType: 'child' });
    ...
    const topScores = await GameScore.find()
      .sort({ score: -1 })
      .limit(10)
      .populate('userId', 'fullName childName email');
